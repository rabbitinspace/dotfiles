...
VOLUME_ICONS=( "%{F$ICON_HEX}%{F-} " )  # Volume icons array, from lower volume to higher
MUTED_ICON=" "  # Muted volume icon
MUTED_SINK_ICON="串 "  # Muted volume icon
MUTED_COLOR="%{F$MUTED_HEX}"  # Color when the audio is muted
NOTIFICATIONS="yes"  # Notifications when switching sinks if enabled
SINK_ICON="%{F$ICON_HEX}串 %{F-}"  # Icon always shown to the left of the default sink names
...
SINK_BLACKLIST=(
    "alsa_output.pci-0000_28_00.1.hdmi-stereo"
)
...
function output() {
    ...
    if [ "$isMuted" = "yes" ]; then
        echo "${MUTED_COLOR}${MUTED_ICON}${curVol}% | ${MUTED_SINK_ICON}${nickname}${END_COLOR}"
    else
